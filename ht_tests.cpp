...
TEST(RemoveTest, EmptyHashTableRemove)
{
	HashTable* ht = createHashTable(hash, BUCKET_NUM);

	EXPECT_EQ(NULL, removeItem(ht, 10));
	EXPECT_EQ(NULL, getItem(ht, 10));
...
TEST(RemoveTest, RemoveFromMiddle)
{
	...
	insertItem(ht, 1, m[0]);     // Bucketnum 1
  	insertItem(ht, 7, m[1]);     // Bucketnum 1
	insertItem(ht, 10, m[2]);      // Bucketnum 1

	void* data = removeItem(ht, 7);	

	EXPECT_EQ(m[1],data);
	EXPECT_EQ(m[0], getItem(ht,1));
	EXPECT_EQ(m[2], getItem(ht,10));

	//free(data);
	destroyHashTable(ht);

	free(m[1]);  // For Edge Case Remove from Middle
}
...
TEST(RemoveTest, RemoveFromHead)
{
	...
	free(m[0]);  // For Edge Case Remove from Head  
}

TEST(RemoveTest, RemoveFromTail)
{
	HashTable* ht = createHashTable(hash,BUCKET_NUM);
	...
	insertItem(ht, 1, m[0]);     // Bucketnum 1.   1%3=1
  	insertItem(ht, 7, m[1]);     // Bucketnum 1      7%3=1
	insertItem(ht, 10, m[2]);      // Bucketnum 1     10%3=1

	void* data = removeItem(ht, 10);	
	...
	free(m[2]);  // For Edge Case Remove from Tail  
}
