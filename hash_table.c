...
void destroyHashTable(HashTable* hashTable) {
  ...
  for (int i=0; i <(hashTable->num_buckets); i++){                // Free Node and Value. 
    ...
    while(location){
      free(location->value);                        // Free the value. Also goes through the entire Hashtable to free the exisiting values.
...
void* insertItem(HashTable* hashTable, unsigned int key, void* value) {
  ...
  if (item){
    ...
    HashTableEntry* CreatedNode = createHashTableEntry(key, value); //Create a pointer of hashtable entry.

    if(CreatedNode == NULL){
    return NULL;  // If Malloc fails Null is returned.  
    }
...
void* removeItem(HashTable* hashTable, unsigned int key) {
  ...
  if(Locator){
    while(current->key != key){                            // We create a while loop which will keep track of the prev variable and current variable this will be changed through out the loop when we shift through the hash table looking at the values. 
    prev=current;
    current= current->next;
    }

    if (prev){                                             // If the prev variable exsists then we will change the pointer to the next value.  
